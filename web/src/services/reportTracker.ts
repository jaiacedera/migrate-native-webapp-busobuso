...
const hasMatchingStatus = (normalizedStatus: string, patterns: string[]): boolean =>
  patterns.some((pattern) => normalizedStatus === pattern || normalizedStatus.includes(pattern));
...
}): Date | null => assignedAt ?? inProgressAt ?? resolvedAt ?? updatedAt ?? createdAtDate;
...
const getInProgressStepDate = ({
  ...
  assignedAt: Date | null;
  createdAtDate: Date | null;
}): Date | null => inProgressAt ?? resolvedAt ?? updatedAt ?? assignedAt ?? createdAtDate;
...
export const buildTrackerSteps = ({
  ...
  const assignedState: TrackerStepState =
    stage === 'assigning'
      ? 'active'
      : stage === 'assigned' || stage === 'rescuing' || stage === 'resolved'
        ? 'done'
        : 'pending';
